var draw = (function () {
  ...
  _createHelpTooltip = function () {
    ...
    var _createPanelInfo = function (type, placeholder) {
        ...
        document.getElementById("drawingPanelInfoLabel").addEventListener("input", function() {    
            let inputUser = document.getElementById("drawingPanelInfoLabel").value;
            document.getElementById("drawTooltip-"+_currentFeature.id_).innerHTML = inputUser;
    ...
    var _addDrawInteraction = function (type) {
        ...
        if(_currentDrawType == type){
            ...
            _drawInt.on("drawstart", function(evt){
                ...
                let inputNameFeature = (geomFeature.getType() === "Point") ? "Repère sans nom" : "Géometrie sans nom";
                ...
                geomFeature.on("change", function (event) {
                    ...
                    if(geomFeature.getType() === "Point") {
                        ...
                        outputMeasureDraw = _measureDrawFormatLength(geomFeature.getCoordinates()[0]);
                        outputMeasureDraw = outputMeasureDraw.concat(' ',_measureDrawFormatArea(_currentFeature));
                    } 
            ...
            _drawInt.on("drawend", function(evt) {
                ...
                if(type === "LineString") {
                    ...
                    if(_getNbPixelsBetweenTwoPoints(geometry.getFirstCoordinate(), geometry.getLastCoordinate()) <= _config.nbPixelsToClosePolygon){
                        let newGeometry = new ol.geom.Polygon([coordinates]);
                        feature.setGeometry(newGeometry);
                    };

                    geometry = feature.getGeometry();
                    let coordinatesLine = geometry.getType() === "LineString" ? geometry.getCoordinates() : geometry.getCoordinates()[0];
                    ...
                    feature.getGeometry().setCoordinates(geometry.getType() === "LineString" ? newCoord : [newCoord]);
                };

                if(type === "Point") {
                    document.getElementById("drawingPanelMesure").innerHTML = ol.coordinate.toStringHDMS(ol.proj.toLonLat(coordinates));;
                };        
    ...
    var _initButtons = function () {
        ...
        if(types.length == 1 && types.every(item => availableTypes.includes(item))){
            ...
            for (const type of types) {
                if (type === 'Point'){
                    buttonOptions = [buttonOptions,'<button id="drawPoint" title="Point" i18n="draw.button.point"',
                    ' class="btn btn-default button-tools btn-raised" onclick="draw.addDrawInteraction(\'Point\');">',
                    '<i class="fas fa-map-pin"></i>',
                    '</button>'].join("");
                }
                if (type === 'LineString'){
                    buttonOptions = [buttonOptions,'<button id="drawLineString" title="Trajet ou Polygone"',
                ' class="btn btn-default button-tools btn-raised" i18n="draw.button.line" onclick="draw.addDrawInteraction(\'LineString\');">',
                '<i class="fas fa-bezier-curve"></i>',
                "</button>",].join("");
                }
                if (type === 'Polygon'){
                    buttonOptions = [buttonOptions,'<button id="drawPolygon" title="Polygone"',
    ...
    var _initDrawTool = function () {
        ...
        _vectorDraw = new ol.layer.Vector({ // New vector
            source: _sourceDraw,
        ...
        _modifyInt.on('modifystart',function(e){
            ...
            _createPanelInfo( _currentFeature.getGeometry().getType(), _currentFeature.getProperties().label);      
            document.getElementById("drawingPanelInfoLabel").value = _currentFeature.getProperties().label;
        });
        ...
        if(_config.snapLayerId){

            $(document).on("layersLoaded", function (e) {
                console.log("Snap ajouté");
                const snap = new ol.interaction.Snap({
                    source: mviewer.getLayer(_config.snapLayerId).layer.getSource(),
                  });
