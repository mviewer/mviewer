...
mviewer.templates.theme = [
    '<li class="{{cls}}" id="theme-layers-{{id}}" >',
        '<a href="#">',
            ...
            '</span><div class="lang" key-lang="{{name}}" style="display:inline;">{{name}}</div>',
        '{{#toggleAllLayers}}',
            '<div class="toggle-theme-layers">',
                '<span class="badge lang" key-lang="Afficher/Masquer toutes les couches de la thématique" title="Afficher/Masquer toutes les couches de la thématique">0/1</span>',
            '</div>',
        '{{/toggleAllLayers}}',
        '</a>',
        ...
        '{{#groups}}',
            '<li class="level-2">',
                '<a href="#"><div class="lang" key-lang="{{title}}" style="display:inline;"> {{title}}</div></a>',
                '<ul class="nav-pills nav-stacked" style="list-style-type:none;">',
                '{{#layers}}',
                    '<li class="mv-nav-item translate" onclick="mviewer.toggleLayer(this);"',
                        ' data-layerid="{{layerid}}"">',
                        '<a href="#" >',
                            '<span class="state-icon far mv-unchecked"></span><div class="lang" key-lang="{{title}}" style="display:inline;">{{title}}</div>',
...
mviewer.templates.layerControl = [
    '<li class="{{cls}}" data-layerid="{{layerid}}" data-title=" {{title}}">',
        '<div class="row layerdisplay-title" >',
            '<a href="#" aria-label="Options" onclick="mviewer.toggleLayerOptions(this);" class="lang" key-lang="Options" title="Options" >',
                '<span class="state-icon glyphicon glyphicon-plus"></span><div class="lang" key-lang="{{title}}" style="display:inline;">{{title}}</div>',
            '</a>',
            '<a href="#" class="close lang" aria-label="close" onclick="mviewer.removeLayer(this)" key-lang="Supprimer" title="Supprimer" >×</a>',
        '</div>',
        ...
        '<div class="row layerdisplay-legend">',
            ...
            '</div>',
            '<canvas class="vector-legend" id="vector-legend-{{layerid}}" width="250" height="0"/>',
            '<img class="mv-legend lang" key-lang="Légende non disponible" {{crossorigin}} id="legend-{{layerid}}" src="{{legendurl}}"',
            ...
            '<div class="row">',
                '<div class="col-md-12">',
                    '<div class="form-group form-group-opacity">',
                        '<label for="{{layerid}}-layer-opacity" class="lang" key-lang="Opacité">Opacité</label>',
            ...
            '<div class="row">',
                '<div class="col-md-12">',
                    '<a data-layerid="{{layerid}}" class="layer-tooltip"',
                        ...
                        '<span class="state-icon far mv-unchecked"></span><div class="lang" style="display:inline;" key-lang="Afficher les infobulles">Afficher les infobulles</div>',
                        '<input type="checkbox" class="hidden" value="false">',
                    '</a>',
                '</div>',
            '</div>',
            ...
            '<div class="row">',
                '<div class="col-md-12">',
                    '<div class="form-group form-group-analyses">',
                        '<label for="{{layerid}}-styles-selector">Analyses</label>',
            ...
            '<div class="row">',
                '<div class="col-md-12">',
                    '<div class="form-group form-group-timer ">',
                        '<label for="{{layerid}}-layer-timefilter">Temporalité</label>',
