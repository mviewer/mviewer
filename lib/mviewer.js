...
mviewer = (function () {
    ...
    var createGetFeatureInfoControl = function () {         
        _map.on('singleclick', function (evt) {
            ...
            var callback = function (result) {               
                var tabs = Array(_queryableLayers.length);
                ...
                $.each(featureInfoByLayer, function (index, response) {
                    ...
                    {                        
                        ...
                        if (html) {                                                      
                            tabs.splice(response.layerinfos.rank,1,'<li><a href="#my-slideshow'+idcarrousel+'" >'+name+'</a></li>');
                            elem =$('<div id="my-slideshow'+idcarrousel +'" ><ul id=slideul'+idcarrousel+' class="bjqs"> </ul> </div>'); 
                            var carrousel_data=$(layerResponse).find(".carrousel li");					  
                            if(carrousel_data.length == 0){
                                carrousel=true;
                                $(elem).find("ul").append('<li>'+layerResponse+'</li>');
                            }
                            else {							
                                for( var i=0; i<carrousel_data.length; i++){						
                                    $(elem).find("ul").append(carrousel_data[i]);
                                    carrousel=true;					 
                                }
                            }
                        }
                            ...
                            if (gml) {
                                ...
                                if (obj.features.length > 0) {                                                                     
                                    tabs.splice(response.layerinfos.rank,1,'<li><a href="#my-slideshow'+idcarrousel+'" >'+name+'</a></li>');
                                    elem=$('<div class="mycarrousel" id="my-slideshow'+idcarrousel +'" ><ul id=slideul'+idcarrousel+' class="bjqs"> </ul> </div>'); 
                                    var oLayer =  _overLayers[obj.features[0].layername];    
                                    carrousel=true;
                                         $(elem).find("ul").append('<li>'+createContentHtml(obj.features, oLayer)+'</li>');
                                      
                                }
                            }
                        }
                         featureInfosHtml.push(elem);
                      }
                    });
                			     
            	if(carrousel && featureInfosHtml.length>0){                    
    ...
    return {     
        ...
        getLegendUrl: function (layer) {
            ...
            return layer.url + '?service=WMS&request=GetLegendGraphic&format=image%2Fpng&width=30&height=20&layer=' + layer.id + '&style=' + layer.style + '&legend_options=fontName:Trebuchet%20MS;fontAntiAliasing:true;fontColor:0x222222;fontSize:10;dpi:96&TRANSPARENT=true';
        	}
        	},
