        ...
        _map.on('singleclick', function (evt) {
            ...
            _map.getFeatureInfo({
                ...
                success: function (featureInfoByLayer) {
                    $("#popup-content").html("");
                    $.each(featureInfoByLayer, function (index, fibl) {
                        var xml = $.parseXML(fibl);
                        var obj = $.xml2json(xml).featureMember;
                        if (obj) {
                            $.each(obj, function (index, o) {
                                var result = o.fid;
                                $("#popup-content").prepend('<strong>' + index + ' : </strong><p><code>' + result + '</code></p>');
                            });
                        }
