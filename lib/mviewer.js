    ...
    var createGetFeatureInfoControl = function () {         
        _map.on('singleclick', function (evt) {
            ...
            for (var i = 0; i < visibleLayers.length; i++) {
                ...
                urls.push({url:url, layername: _overLayers[visibleLayers[i].getSource().getParams()['LAYERS']].name});
            }            
            ...
            var callback = function (result) {
                ...
                $.each(featureInfoByLayer, function (index, response) {
                    ...
                    {                        
                        ...
                        if (html) {
                            NbFeatureInfo+=1;
                            $("#ulnav").append('<li><a href="#my-slideshow'+idcarrousel+'">'+name+'</a></li>')
                ...
                for (var request, i = -1; request = urls[++i];) {
                    requests.push($.ajax({
                        ...
                        success: function (response) {
                            featureInfoByLayer.push({response:response,title:this.layername});
                        }
                    }));
                }
            };
