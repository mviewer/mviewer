...
/*
 * Global array where you can hook function for click on specific layer
 */
var Layers_Hooks = new Array();
/*
 *
 */
...
mviewer = (function () {
    ...
    var createGetFeatureInfoControl = function () {         
        _map.on('singleclick', function (evt) {
            ...
            /*
             * Début variante Layers_Hooks :
             *    On cherche la première couche interrogeable
             *    si cette couche possède une fonction spécifique, on lance cette fonction
             */
             var flag_hook = false;
             $.each( _queryableLayers, function(i, layer) {
                   if (layer.getVisible())
                   {
                     // La couche est affichée
                     if (typeof Layers_Hooks[layer.get('id')] === 'function')
                     {
                       // Il existe un HOOK
                       Layers_Hooks[layer.get('id')](coordinate, _projection.getCode(), layer); // On exécute la fonction
                       flag_hook = true;
                       return false; // On s'arrête au premier dans le "each"
                     }
                   }
                }
             );
             /*
              * Si on n'a rien trouvé, on cherche un hook par defaut
              */
             if ((!flag_hook) && (typeof Layers_Hooks['defaut'] === 'function'))
             {
                 Layers_Hooks['defaut'](coordinate, _projection.getCode(), null);
                 flag_hook = true;
             }
             /*
              *  Si on a un contenu on s'arrête là
              */
             if (flag_hook)
             {
               mobileHide();
...
/* Ancienne version */
                ...
                for (var request, i = -1; request = urls[++i];) {
                    requests.push($.ajax({
                        ...
                        success: function (response) {
                            featureInfoByLayer.push(response);
                        }
                    }));
                }
            };
    ...
    var _initDataList = function () {
        ...
        $.each(_themes, function (id, theme) {  
            reverse_themes.push(theme);
                ...
                var typeFondCarte = $(_options).find("baselayers").attr("style");
                /* On surchage eventuellement par les params de l'URL */
                if (config.blstyle != "config") {
                  typeFondCarte = config.blstyle;
                }
                if(typeFondCarte === "integrated"){
                        htmlListGroup +=  new TPLLayergroup("Fonds de carte", document.getElementById("backgroundlayerstoolbar").innerHTML, false).html();
                }else if(typeFondCarte === "gallery_simple"){
                         $("#backgroundlayerstoolbar").toggle();
                         return;
                }else if(typeFondCarte === "tab"){
                         $("#backgroundlayerstoolbar").toggle();
                }else{
                         $("#backgroundlayerstoolbar").toggle();
                }
		...
		$( ".metadata-btn" ).each(function() {		
			$( this ).bind( "click",function() {
                                ...
                                if ((md.local != null) && (md.local != '')) {
                                        $.ajax({
                                                url: md.local+'?blob='+Date.now(),
                                                success: function (data) {
                                                        $("#metadata-local").html(data);
                                                }
                                        });
                                }

                                if (((md.csw != null) && (md.csw != '')) || ((md.url != null) && (md.url != '')))
                                $("#metadata-abstract").text("Récupérations des informations en ligne ...");

                                if ((md.url != null) && (md.url != ''))
                                $("#metadata-target").attr("href",md.url);

                                if ((md.csw != null) && (md.csw != ''))

				$.ajax({
					url: md.csw,				  				  
                                        error: function (err) {
                                                $("#metadata-abstract").text("Echec récupérations des informations en ligne ...");
                                                $("#metadata-target").attr('href','#');
                                        },
                    success: function (data) {	
    ...
    var _parseGML = function (gml) {
        var obj = $.xml2json(gml);
        var o = {features: []};        
        for(var prop in obj.featureMember) {
            if(obj.featureMember.hasOwnProperty(prop))
                o.features.push({layername:prop, attributes:obj.featureMember[prop]});                
        }        
        return o;
    };
    ...
    return {     
        ...
        setBaseLayer: function (baseLayerId) {
            ...
            $.each(_backgroundLayers, function (id, layer) {
                ...
                /* Surcharge du style de baselayers par l'URL */
                if (config.blstyle != "config") opt = config.blstyle;
        ...
        getLayerMetadata: function (id) {
            return {url:_overLayers[id].metadata, csw:_overLayers[id].metadatacsw, local:_overLayers[id].metadatalocal};
        },
        ...
        getLegendUrl: function (layer) {
            ...
            return layer.url + '?service=WMS&request=GetLegendGraphic&format=image%2Fpng&width=20&height=20&layer=' + layer.id + '&style=' + layer.style + '&legend_options=fontName:Trebuchet%20MS;fontAntiAliasing:true;fontColor:0x222222;fontSize:10;dpi:96&TRANSPARENT=true';
        },
        ...
        init: function (xml) {
           // try {
                ...
                var baselayerControlStyle = $(xml).find('baselayers').attr("style");
                                /* Surcharge du style de baselayers par l'URL */
                if (config.blstyle != "config") baselayerControlStyle = config.blstyle;
                ...
                if (config.wmc) {
                    ...
                    $(themes.get().reverse()).each(function () {
                        ...
                        $(layersXml.get().reverse()).each(function () {
                            ...
                            oLayer.metadatalocal = $(this).attr("metadata-local");
                            ...
                            oLayer.enable = true;
                            oLayer.legend = ($(this).attr("legend") === "false") ? false : true;
