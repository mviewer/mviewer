    ...
    var createGetFeatureInfoControl = function () {        
        _map.on('singleclick', function (evt) {
			...
			var html = null; 
                        ...
                        if (html && (html.search('<!--nodatadetect-->\n<!--nodatadetect-->')<0)) {                            
                            ...
                            if (gml) {
                                ...
                                if (obj.features.length > 0) {
                                    var oLayer =  _overLayers[obj.features[0].featureId_.split(".").slice(0, -1).join(".")];                               
    ...
    return {     
           ...
           // try {
                ...
                var zoom = 8;
                 if (config.x && config.y && config.z) {
                    center =   [parseFloat(config.x), parseFloat(config.y)];       
                    zoom = parseInt(config.z);
                 } 
                _map = new ol.Map({
                    target: 'map',
                    controls: [
                ...
                $(themes.get().reverse()).each(function () {
                    ...
                    $(layersXml.get().reverse()).each(function () {
                        ...
                        oLayer.default = ($(this).attr("visible") == "true") ? true : false;
        ...
        restoreInitialDisplay: function () {      
            $.each(_overLayers, function (id, layer) {
                if  (layer.checked != layer.default) {
                    layer.checked = layer.default;                    
                    layer.layer.setVisible(layer.default);
                    $("#datalist").find('input[name="'+layer.id+'"]').prop( "checked", layer.default).checkboxradio( "refresh" );
                }
            });            
        },
