...
mviewer = (function () {
    ...
    var _pointerMoveHandler = function(evt) {
      ...
      /** @type {string} */
      var helpMsg = 'Click débuter le tracé';

      if (_sketch) {
        var geom = (_sketch.getGeometry());
        if (geom instanceof ol.geom.Polygon) {
          helpMsg = 'Click pour poursuivreg le polygone';
      ...
      _draw.on('drawstart',
          function(evt) {
            ...
            listener = _sketch.getGeometry().on('change', function(evt) {
              ...
              var output;
              if (geom instanceof ol.geom.Polygon) {
                output = _formatArea(/** @type {ol.geom.Polygon} */ (geom));
        ...
        if (visibleLayers.length > 0) {
            ...
            $.ajax({
                ...
                error: function (xhr, ajaxOptions, thrownError) {
                    alert("Problème avec l'instance Elasticsearch.\n" +  thrownError + "\n Désactivation du service." );
        ...
        init: function (xml) {
           // try {
                ...
                if (applicationOverride.attr("logo")) {
                    $("#panel-header img").attr("src", logo);
                }
