...
mviewer = (function () {
    ...
    var _getlegendUrl = function (layer) {
         return layer.url + '?service=WMS&Version=1.3.0&request=GetLegendGraphic&SLD_VERSION=1.1.0'+
            '&format=image%2Fpng&width=30&height=20&layer=' + layer.layername + '&style=' + layer.style +
    ...
    var createContentHtml = function (features, olayer) {
        ...
        features.forEach(function (feature) {
            var nbimg = 0;
            counter+=1;            
            var li = '<li><div class="popupitem" style="padding-left: 25px;" ><h3>'+olayer.name+' : <font size=-1>' + feature.attributes[olayer.fields[0]] +'</font></h3>';           
                ...
                if (config.wmc) {                    
                    ...
                    $(themes.get().reverse()).each(function () {                        
                        ...
                        $(layersXml.get().reverse()).each(function () {                            
                            ...
                            l.setVisible(false);
                            l.setOpacity(oLayer.opacity);
                            if (oLayer.scale && oLayer.scale.max) { l.setMaxResolution(_convertScale2Resolution(themeLayers[oLayer.id].scale.max)); }  
                            if (oLayer.scale && oLayer.scale.min) { l.setMinResolution(_convertScale2Resolution(themeLayers[oLayer.id].scale.min)); } 
                            l.set('name', $(this).attr("name"));
                            l.set('mviewerid', mvid);
                            themeLayers[oLayer.id].layer = l;
                            _themes[themeid].layers[oLayer.id] = themeLayers[oLayer.id];
                            if (themeLayers[oLayer.id].queryable) {
                                _queryableLayers.push(l);
                            }
                            if (themeLayers[oLayer.id].searchable) {
                                _searchableLayers.push(l);
                            }
                            if (themeLayers[oLayer.id].scale) {
                                _scaledDependantLayers.push(themeLayers[oLayer.id]);
                            }
                            _overLayers[oLayer.id] = themeLayers[oLayer.id];
                            _map.addLayer(l);                        
                        }); //fin each	layer					
