            ...
            if ($(this).attr('hidden')!='1' && $(this).attr('queryable')=='1') {
                ...
                if (oLayer.metadata.search('geonetwork') > 1) {
                    ...
                    oLayer.metadatacsw = oLayer.metadata.substring(0,oLayer.metadata.search('geonetwork')) +
                        '/srv/eng/csw?SERVICE=CSW&VERSION=2.0.2&REQUEST=GetRecordById&elementSetName=full&ID=' +
                        mdid;
                }               
                ...
                if (config.wmc) {
                    $.ajax({
                        ...
                        success: function (xml) {
                            ...
                            _themes['wmc'].id = 'wmc';
                            _themes['wmc'].name = 'Données de contexte';                   
                            _themes['wmc'].layers = {};                            
                            var wmc = _parseWMCResponse(xml);
                            themeLayers = wmc[0];
                            _map.getView().fitExtent(wmc[1], _map.getSize());
                            _themes['wmc'].layers = themeLayers;
                            _initDataList();
                        }
                    });    
                }
