        ...
        var visibleLayers = [];
            ...
            for (var i = 0; i < visibleLayers.length; i++) {
                ...
                if (layer && featureid) {
                    ...
                    url+= '&CQL_FILTER='+_elasticSearchLinkid+'%3D%27'+featureid+'%27';
                }                    
		...
		$.each(reverse_themes.reverse(), function (id, theme) {            
			...
			$.each(reverse_layers.reverse(), function (id, layer) {
				htmlListLayers += new TPLLayercontrol(layer.id, layer.name, layer.legendUrl, 
					layer.queryable, layer.checked, crossorigin).html();
			});
    ...
    return {     
        ...
        init: function (xml) {
           // try {                
                ...
                $("#adresse-attibution").text($(xml).find('olscompletion').attr("attribution"));
        ...
        print() {
            if ((_legendRenderer === "canvas")&&($("#maplegend-popup").css("visibility") === "visible")) {
                $.post('print.php',
                    {  
