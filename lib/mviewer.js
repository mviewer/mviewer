            ...
            var callback = function (result) {
                ...
                $.each(featureInfoByLayer, function (index, response) {
                    ...
                    {                        
                        ...
                        if (html) {
                            NbFeatureInfo+=1;
                            $("#ulnav").append('<li data-sort="'+response.layerinfos.rank+'" ><a href="#my-slideshow'+idcarrousel+'" >'+name+'</a></li>')
                            $("#selector").append('<div id="my-slideshow'+idcarrousel +'" ><ul id=slideul'+idcarrousel+' class="bjqs"> </ul> </div>'); 
                            var carrousel_data=$(layerResponse).find(".carrousel li");					  
                            if(carrousel_data.length == 0){
                                carrousel=true;
                                $("#slideul"+idcarrousel).append('<li>'+layerResponse+'</li>');
                            }
                            else {							
                                for( var i=0; i<carrousel_data.length; i++){						
                                    $("#slideul"+idcarrousel).append(carrousel_data[i]);
                                    carrousel=true;					 
                                }
                            }
                        }
                            ...
                            if (gml) {
                                ...
                                if (obj.features.length > 0) { 
                                    NbFeatureInfo+=1;
                                    $("#ulnav").append('<li data-sort="'+response.layerinfos.rank+'" ><a href="#my-slideshow'+idcarrousel+'" >'+name+'</a></li>')
                                    $("#selector").append('<div class="mycarrousel" id="my-slideshow'+idcarrousel +'" ><ul id=slideul'+idcarrousel+' class="bjqs"> </ul> </div>'); 
                                    var oLayer =  _overLayers[obj.features[0].layername];    
                                    carrousel=true;
                                         $("#slideul"+idcarrousel).append('<li>'+createContentHtml(obj.features, oLayer)+'</li>');
            	...
            	if(carrousel && NbFeatureInfo>0){
                    //reorder items in #ulnav to respect order layer in themepanel
                    var neworder = $('#ulnav li').sort(function (a, b) {
                          var contentA =parseInt( $(a).attr('data-sort'));
                          var contentB =parseInt( $(b).attr('data-sort'));
                          return (contentA > contentB) ? -1 : (contentA < contentB) ? 1 : 0;
                    });
                    $("#ulnav").html("").append(neworder);                   
                    $('#popup').show();
                    for(i=1;i<=idcarrousel;i++){
                        $('#my-slideshow'+i).bjqs({
                            ...
                            centermarkers : true,
                            nexttext : 'Suiv', // Text for 'next' button (can use HTML)
                            prevtext : 'Prec' 
                        });
                    }                    
                    $( "#selector" ).tabs();
