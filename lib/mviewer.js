mviewer = (function () {
    ...
    var createContentHtml = function (features, title, maxheight) {
        ...
        features.forEach(function (feature) {
            ...
            html += '<div class="popupitem"><h3>'+olayer.name+'</h3>';
                ...
                if (attributes[f]) { // si valeur != null
                    if (attributes[f].indexOf("http://") >= 0) {
                        if (attributes[f].toLowerCase().indexOf(".jpg") >= 0) {                                                       
                            html += '<a onclick="map.mviewer.popupPhoto(\'' +  attributes[f] + '\')" ><img class="popphoto" src="' + attributes[f] + '" alt="image..." style="height:100px;"></a>';
        ...
        $(themes).find("theme").each(function () {
            ...
            $(this).find("map").each(function () {
                ...
                html += ['<li id="' + $(this).attr("id") + '_btn">',
                    '<a href="#" data-mini="false" data-iconpos="left" onclick="map.mviewer.loadMap(\'' + $(this).attr("id") + '\');">',
                    $(this).attr("name") + '</a></li>'
                ].join("");
            });
        });
    ...
    return {
        ...
        init: function (xml) {
            try {
                ...
                var panelbutton = ['<a id="data_btn" data-mini="true"  data-iconpos="left" data-icon="grid" href="#themepanel" ',
				'title="Liste des données disponibles" data-theme="a" data-rel="panel" data-role="button" >Les données</a>'].join("");
                if (baselayerControlStyle === "default") {
                    $("#extras").remove();
                    $("#baselayerstoolbar").prepend(panelbutton);
