...
mviewer = (function () {
    ...
    var _initDataList = function () {
		...
		$.each(reverse_themes.reverse(), function (id, theme) {            
			...
			$.each(reverse_layers.reverse(), function (id, layer) {
				htmlListLayers += new TPLLayercontrol(layer.id, layer.name, layer.legendUrl, 
					layer.queryable, layer.checked, crossorigin, layer.searchable, layer.customcontrol).html();
			});
        ...
        $( ".customcontrol-btn" ).each(function() {		
			$( this ).bind( "click",function() {			
				console.log(this.name);
                $("#customcontrol-title").text(this.name);
                $("#customcontrol-content div").remove();
                $("#customcontrol-content").append(_overLayers[this.name].form);
                $("#customcontrol-popup").popup('open');
			});
		});
    ...
    return {
        ...
        setLayerLegend : function (layerDefinition) {
            var legendUrl = _getlegendUrl(layerDefinition);
            return legendUrl;
        },
                    ...
                    $(themes.get().reverse()).each(function () {                        
                        ...
                        $(layersXml.get().reverse()).each(function () {                            
                            ...
                            if (oLayer.customcontrol) {
                                 $.ajax({
                                    url: 'controls/'+oLayer.id+'.html',
                                    layer: oLayer.id,
                                    dataType: "text",
                                    success: function (html) {
                                        _overLayers[this.layer].form = html;                                        
                                    }
                                });
                                $.ajax({
