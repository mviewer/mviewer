mviewer = (function () {
    ...
    var _initDataList = function () {
		...
		$.each(_themes, function (id, group) {
			var htmlListLayers = ""
			$.each(_themes[group.id].layers, function (id, layer) {
				htmlListLayers += new TPLLayercontrol(layer.id, layer.name, _map.mviewer.getLegendUrl(layer) , layer.queryable, layer.checked, layer.enabled).html();
			});
			htmlListGroup +=  new TPLLayergroup(group.name, htmlListLayers, group.collapsed).html();
    ...
    return {        
        ...
        changeLayerOpacity: function (id, value) {
            _allOverLayers[id].layer.setOpacity(value / 100);
        },
        ...
        getLayerOpacity: function (id) {
            return _allOverLayers[id].layer.opacity * 100;
        },
        ...
        getLayerMetadata: function (id) {
            return {url:_allOverLayers[id].metadata, csw:_allOverLayers[id].metadatacsw};
        },
        ...
        toggleOverLayer: function (id) {
            _allOverLayers[id].checked = !_allOverLayers[id].checked;
            if (_allOverLayers[id].checked) {                
                _allOverLayers[id].layer.setVisibility(true);
            } else {                
                _allOverLayers[id].layer.setVisibility(false);
            }
            /*_map.events.triggerEvent('mviewer.toggleoverlayer', {
                layer: _allOverLayers[id]
            });*/
        },
            ...
            try {
                ...
                $(bl).each(function () {
                    ...
                    if (baselayerControlStyle === "gallery") {
                        domBaseLayer = '<li onclick="map.mviewer.setBaseLayer(\'' + id + '\')"><a id="' + id + '_btn" href="#"><img src="' + thumb + '" /><h3>' + label + '</h3><p>' + $(this).attr("title") + '</p></a></li>';
                ...
                $(maps).each(function () {
                    ...
                    _themes[mapid] = {};
					_themes[mapid].collapsed = $(this).parent().attr("collapsed");
                    _themes[mapid].id = mapid;
                    _themes[mapid].name = $(this).attr("name");                    
                    _themes[mapid].layers = {};
                    ...
                    $(layersXml.get().reverse()).each(function () {
                        ...
                        _themes[mapid].layers[layerId] = themeLayers[layerId];
                        if (themeLayers[layerId].queryable) {
                            _queryableLayers.push(themeLayers[layerId].layer);
                        }
                        _allOverLayers[layerId] = themeLayers[layerId];
