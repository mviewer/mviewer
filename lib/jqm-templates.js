...
function TPLLayercontrol(layerid, title, legendurl, queryable, checked, crossorigin, searchable, opacity, styles){   
...
TPLLayercontrol.prototype.html = function(){                    
     ...
     ' data-legendurl="'+this.legendurl+'" data-checked="'+this.checked+'" data-crossorigin="'+this.crossorigin+'" data-styles="'+this.styles+'" data-opacity="'+this.opacity+'" data-searchable="'+this.searchable+'" >',
     '<a href="#" ><span class="state-icon glyphicon glyphicon-unchecked"></span>',     
					this.title,	
					'<input type="checkbox" class="hidden" value="false" ></a></li>'].join( " ");
}

function TPLLayercontrol2(layerid, title, legendurl, queryable, checked, crossorigin, searchable, opacity, styles){   
   this.layerid = layerid;
   this.title = title;   
   this.queryable = queryable;
   this.legendurl = legendurl;
...
TPLLayercontrol2.prototype.html = function(){
    ...
    if (this.styles && this.styles != "undefined") {
        ...
        for (var i=0; i<styles_array.length; i++) { 
            options.push('<option label="'+styles_array[i]+'" value="'+styles_array[i]+'"></option>');
        }
