...
TPLBackgroundlayerstoolbar.prototype.init = function(){
    ...
    if (this.option === "gallery") {
        ...
        $("#backgroundlayerstoolbar-gallery").append(['<div class="" onclick="mviewer.bgtoogle();">',
            '</div><ul id="basemapslist" ></ul>'].join(""));
    }
    return this;
}
...
TPLBackgroundLayerControl.prototype.html = function(){
    var template = "";
    if (this.option === "gallery") {
        template = ['<li data-original-title="'+this.layer.attr("label")+'" title="'+this.layer.attr("label")+'" ',
        'style="background:url(\''+this.layer.attr('thumbgallery')+'\') center bottom/60px no-repeat;" ',
        'onclick="mviewer.setBaseLayer(\'' +  this.layer.attr("id") + '\')">',
        '<a id="' + this.layer.attr("id") + '_btn" href="#" >',
        '</a></li>'].join(" ");
        $("#basemapslist").append(template);
    }
}

function TPLLayergroupGroup(title, grouplist, id, icon, toggleAllLayers){
    this.title = title;
    this.grouplist = grouplist;
    this.id = id;
    this.icon = icon;
    this.toggleAllLayers = toggleAllLayers;
}
...
TPLLayergroupGroup.prototype.html = function(){
    ...
    var toggleBtn = "";
    var toggleCls = "";
    if (this.toggleAllLayers) {
        toggleBtn = '<div class="toggle-theme-layers">' +
        '<span class="badge" title="Afficher/Masquer toutes les couches de la thématique">0/1</span></div>';
        toggleCls = "empty";
    }
    return ['<li class="level-1 '+toggleCls+'" id="theme-layers-'+this.id+'" >',
    '<a href="#"><span class="fa-stack fa-lg pull-left">',
    '<i class="fa '+faicon+' fa-stack-1x "></i></span>'+this.title+toggleBtn+'</a>',
    '<ul class="nav-pills nav-stacked" style="list-style-type:none;">',
    htmlGroups.join(""),
    '</ul>',
    '</li>'].join(" ");
...
function TPLLayergroup(title, layerlist, id, icon, toggleAllLayers){
    this.title = title;
...
TPLLayergroup.prototype.html = function(){
    ...
    return ['<li class="'+toggleCls+'" id="theme-layers-'+this.id+'" >',
    '<a href="#"><span class="fa-stack fa-lg pull-left">',
    '<i class="fa '+faicon+' fa-stack-1x "></i></span>'+this.title+toggleBtn+'</a>',
    '<ul class="nav-pills nav-stacked" style="list-style-type:none;">',
    this.layerlist,
...
function TPLLayercontrol(layer){
    this.data = {
        ...
        customcontrol : layer.customcontrol
    }
}
...
function TPLLayercontrol2(layer){
    ...
    this.customcontrol = layer.customcontrol;
}
...
TPLLayercontrol2.prototype.html = function(){
    ...
    if (this.customcontrol && mviewer.customControls[this.layerid]) {
        custom_controls = mviewer.customControls[this.layerid].form;
    }

    if (this.type === 'customlayer' && this.tooltip) {
    ...
    //if (this.summary && this.summary.length>=3) {
        if (this.metadata || this.summary) {
            ...
            ' title="Résumé" data-content=""><span class="glyphicon glyphicon-info-sign" aria-hidden="true">',
            '</span></a>'].join ("");
        }
    ...
    if (this.attributefilter && this.attributevalues != "undefined" && this.attributefield != "undefined") {
        ...
        for (var i=0; i<attributes_array.length; i++) {
            options_attributes.push('<option label="'+attributes_array[i]+'" value="'+attributes_array[i]+'">'
              +attributes_array[i]+'</option>');
        }
    ...
    return  ['<li class="list-group-item mv-layer-details '+draggable+'" data-layerid="'+this.layerid+'" data-title="',
    this.title+'" >',
    ...
    '<input type="text" class="mv-slider" id="'+this.layerid+'-layer-opacity" ',
    'name="'+this.layerid+'" value="'+this.opacity+'"',
    'data-provide="slider" data-slider-value="'+this.opacity+'" data-slider-max="1" ',
...
TPLInfopanel.prototype.html = function () {
    ...
    '<a class="left carousel-control" href="#carousel-'+this.panel+'-'+this.id_tab[this.panel]+'" ',
    'role="button" data-slide="prev">',
    '<span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span>',
    '<span class="sr-only">Previous</span>',
    '</a>',
    '<a class="right carousel-control" href="#carousel-'+this.panel+'-'+this.id_tab[this.panel]+'" ',
