...
TPLBackgroundLayerControl.prototype.html = function(){
	...
	if (this.option === "gallery") {
		template = ['<li style="background:url(\''+this.layer.attr('thumbgallery')+'\') center bottom no-repeat;" onclick="mviewer.setBaseLayer(\'' +  this.layer.attr("id") + '\')">',
			...
			//'<h6>' +  this.layer.attr("label") + '</h6>',
			//'<p>' + this.layer.attr("title") + '</p>',
            '</p></a></li>'].join(" ");
...
TPLLayercontrol.prototype.html = function(){
	/*return  ['<li data-layer="'+this.layerid+'" data-theme="a" data-icon="false"><a href="#">',
     ...
     return  ['<li data-layer="'+this.layerid+'" "><a href="#"> ',
					this.title,	
					'</a></li>'].join( " ");
}
