...
TPLBackgroundLayerControl.prototype.html = function(){
	var template = "";
	if (this.option === "gallery") {
		template = ['<li data-original-title="'+this.layer.attr("label")+'" title="'+this.layer.attr("label")+'" style="background:url(\''+this.layer.attr('thumbgallery')+'\') center bottom/60px no-repeat;" ',
...
TPLLayergroupGroup.prototype.html = function(){
    ...
    if (this.toggleAllLayers) {
        toggleBtn = '<div class="toggle-theme-layers"><span class="badge" title="Afficher/Masquer toutes les couches de la thématique">0/1</span></div>';
        toggleCls = "empty";
    }
    return ['<li class="level-1 '+toggleCls+'" id="theme-layers-'+this.id+'" >',
        '<a href="#"><span class="fa-stack fa-lg pull-left"><i class="fa '+faicon+' fa-stack-1x "></i></span>'+this.title+toggleBtn+'</a>',
           '<ul class="nav-pills nav-stacked" style="list-style-type:none;">',
                htmlGroups.join(""),
           '</ul>',
    '</li>'].join(" ");
	...
	return  ['<li class="list-group-item mv-layer-details '+draggable+'" data-layerid="'+this.layerid+'" data-title="'+this.title+'" >',
						'<div class="row layerdisplay-title" >',
                         ...
                         '<div class="row layerdisplay-legend">',
                             ...
                             'alt="Légende non disponible" onload="mviewer.legendSize(this)" onError="this.onerror=null;this.src=\'img/nolegend.png\';"/>',                             
                         '</div>',
