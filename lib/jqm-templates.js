...
TPLLayergroup.prototype.html = function(){
	return ['<div class="mv-theme" data-theme="b" data-role="collapsible" data-collapsed="'+this.collapsed+'">',
...
function TPLLayercontrol(layerid, title, legendurl, queryable, checked, crossorigin, searchable, customcontrol){   
   ...
   if (customcontrol === true) {
        this.customcontrol = '<a class="customcontrol-btn ui-btn ui-shadow ui-corner-all ui-icon-edit ui-btn-icon-notext ui-btn-a ui-btn-inline" id ="customcontrol-btn-'+this.layerid+'" name="'+this.layerid+'" href="#"  title="Extra operations" ></a>';
   }
   //this.customcontrol = (customcontrol===false)? 'style="display: none;"': '';
}

TPLLayercontrol.prototype.html = function(){
	return  ['<li data-layer="'+this.layerid+'" data-theme="a" data-icon="false"><a href="#">',
						...
						'<label for="checkbox-mini-'+this.layerid+'">',
							'<table style="width:100%;" >',
								'<tbody>',
									'<tr>',
										'<td><img class="mv-legend" '+this.crossorigin+' id="legend-'+this.layerid+'" src="'+this.legendurl+'" alt="Légende non disponible" onError="this.onerror=null;this.src=\'img/nolegend.png\';"></td>',
										(this.searchable===true)?'<td class="layer-searchable"></td>':'',
									'</tr>',                            
								'</tbody>',
							'</table>',                
						'</label>',
